...
class OutboxEventRepositoryTest {
    ...
    void save_publishedDefaultsToFalseWithNullPublishedAt() {
        OutboxEvent saved = repository.save(buildEvent("OnboardingStartedV1"));
    ...
    void findByPublishedFalse_excludesPublishedEvents() {
        OutboxEvent unpublished = repository.save(buildEvent("OnboardingStartedV1"));

        OutboxEvent published = buildEvent("OnboardingStartedV1");
    ...
    private OutboxEvent buildEvent(String eventType) {
        ...
        event.setAggregateType("OnboardingRequest");
        event.setAggregateId(UUID.randomUUID());
        event.setEventType(eventType);
        event.setPayload("{\"requestId\":\"" + UUID.randomUUID() + "\"}");
        return event;
    }
